data = [{"name": "An Giang", "population": 2153700, "area": 3536.7, "senator": 10},
...
def resolve(data:dict):
    ...
    for entry in data:
        newKey = entry["name"]
        newValue = entry["population"]
        if(newValue > 1000000) :
            dictProvinces.update({newKey: newValue})
    sortedDictProvinces = sorted(dictProvinces, key=dictProvinces.get, reverse=True)

    for i in sortedDictProvinces:
        result.append({i:dictProvinces[i]})
